module Ploto
  describe HorizontalAxis do
    describe "#render" do
      let(:axis) { HorizontalAxis.new(1,5, pixel_width: 100) }

      it "renders the border line, tick marks, and tick labels" do
        xml = axis.render(50, 100)
        root = xml.root
        expect(root[:width].to_i).to eq 100
        expect(root[:height].to_i).to eq (HorizontalAxis::CHARACTER_HEIGHT / 2.0) + HorizontalAxis::TICK_HEIGHT + HorizontalAxis::TICK_PADDING
      ...
      context "with an axis label" do
        ...
        it "renders the label" do
          xml = axis.render(50, 100)
          root = xml.root
          expect(root[:height].to_i).to eq HorizontalAxis::CHARACTER_HEIGHT + HorizontalAxis::TICK_HEIGHT +
                                           HorizontalAxis::TICK_PADDING + HorizontalAxis::LABEL_TOP_PADDING
