module Ploto
  module Axes
    describe NumericHorizontalAxis do
      describe "#render" do
        let(:axis) { NumericHorizontalAxis.new(1,5, pixel_width: 100) }

        it "renders the border line, tick marks, and tick labels" do
          xml = axis.render(50, 100)
          root = xml.root
          expect(root[:width].to_i).to eq 100
          height = (Ploto::CHARACTER_HEIGHT / 2.0) + 
                    NumericHorizontalAxis::TICK_HEIGHT + NumericHorizontalAxis::TICK_PADDING
